  ...
  framework-sync:
    ...
    steps:
      - name: Check out downstream project
        ...
        with:
          fetch-depth: 0
          token: ${{ secrets.WP_CORE_BASE_AUTOMATION_TOKEN || secrets.GITHUB_TOKEN }}

      - name: Report automation token fallback
        env:
          WP_CORE_BASE_AUTOMATION_TOKEN: ${{ secrets.WP_CORE_BASE_AUTOMATION_TOKEN }}
        run: |
          if [ -z "${WP_CORE_BASE_AUTOMATION_TOKEN}" ]; then
            echo "::warning::WP_CORE_BASE_AUTOMATION_TOKEN is not configured; framework update PRs created with GITHUB_TOKEN may not trigger required pull request checks."
          fi
