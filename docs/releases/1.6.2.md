...
This is the `patch` framework release from `v1.6.1` to `v1.6.2` for `wp-core-base`.

It publishes the current validated framework state for downstream adoption and framework self-update PRs.

## Downstream Impact

Downstream repositories pinned to an older `wp-core-base` release can update to `v1.6.2` through `framework-sync` or by vendoring the published snapshot manually.
...
No special migration steps are expected by default for `v1.6.2`.
...
No framework-managed downstream workflow or pipeline template changes are expected by default for `v1.6.2`.
...
No extra downstream action is required by default for `v1.6.2`.
...
Normal release flow: run `prepare-wp-core-base-release`, review and merge `release/v1.6.2`, then let `finalize-wp-core-base-release` create the tag and publish the authoritative framework release artifact.
