...
The CI validation jobs retain `test-report.json` for every PHP version and `coverage-observed.json` for PHP 8.4 in the workflow run's **Artifacts** section. Artifact names include the PHP version and run attempt, and retention is 14 days. Uploads run after failures too, preserving any reports the suite produced; a failure before report generation produces a missing-files warning without replacing the original failure. Only these two report paths are uploaded. Coverage records observed framework lines in the test process, excluding subprocess execution; it is not a complete coverage percentage. Download evidence before expiry when retaining it for a release review.
