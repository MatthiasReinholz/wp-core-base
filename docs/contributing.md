...
The CI validation jobs retain `test-report.json` for every PHP version and `coverage-observed.json` for PHP 8.4 in the workflow run's **Artifacts** section. Artifact names include the PHP version and run attempt, and retention is 14 days. Uploads run after failures too, preserving any reports the suite produced; a failure before report generation produces a missing-files warning without replacing the original failure. Only these report paths and the verified historical-input receipt are uploaded; release ZIPs are excluded. Coverage records observed framework lines in the test process, excluding subprocess execution; it is not a complete coverage percentage. Download evidence before expiry when retaining it for a release review.

The required PHP 8.4 validation job also downloads the published v1.4.8 installer fixture. `scripts/ci/prepare_legacy_release.php` verifies its pinned SHA-256, filename-bound checksum and detached signature against the checked-in official keyring before any old code executes. A failed download or verification fails the job. The runner sets `WP_CORE_BASE_LEGACY_RELEASE_ARTIFACT` explicitly, enabling the historical installer lane for both repository profiles and both automation hosts, plus current-installer acceptance of the historical full snapshot. The v1.4.8 launcher still needs the documented executable-mode repair. `legacy-release-verification.json` records the verified input; `test-report.json` records the compatibility suite outcome. This tests installer/package compatibility, not live remote PR operations or a production site's database migration.
